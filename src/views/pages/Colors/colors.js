...
import { deleteColor, getColors } from "services/colorServices";
import ConformationModal from "components/common/ConformationModal";
import { Edit3, Plus, Trash } from "react-feather";
import ColorsAddModal from "./colorsAddModal";
...
const Colors = props => {
  ...
  const getColorsList = useCallback(async () => {
    fetching();
    ...
    () => [
      {
        ...
        Cell: tableInstance => (
          <span className="text-capitalize">
            {tableInstance.row.values.color}
          </span>
        )
      },
  ...
  const deleteClick = () => {
    ...
    deleteColor(token, { id: deleteId }).then(res => {
      if (res.success) {
        toggleRefresh(true);
